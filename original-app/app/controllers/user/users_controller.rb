class User::UsersController < ApplicationController
    ...
    def show
        ...
        @commentIds = Comment.where(micropost_id: @microposts.ids).select(:id)
     
        session[:comment_id] =  nil
        @commentId = []
#        @commentIds.each do |id|
#             @commentId << @user.encryptor(id)
 #       end
        @i = 0
        
        @commentIdd = []
        @commentId.each do |id|
            @commentIdd << @user.decrypt(id)
        end
    ...
    def user_params
        params.require(:user).permit(:first_kanji,:last_kanji,:first_kana,:last_kana,:first_name , 
            ...
            member_attributes:[:country,:prefecture,:city,:town_area,:address,:building,:room,:datetime,]
        )
    end
